...
int main(int argc,char* argv[])
{
    ...
    */
    ...
    while(server_running)
    {
        ...
        LOG_INFO("%s  %d", "epoll_fd --> ", epoll_fd);
        epoll_wait_ret_value=epoll_wait(epoll_fd,events,MAX_EVENT_NUM,-1);

        LOG_INFO("%s    %d", "epoll_wait_ret_value", epoll_wait_ret_value);
        ...
        for(int i=0;i<epoll_wait_ret_value;++i)
        {
            ...
            LOG_INFO("%s %d", "sock_fd --> ", sock_fd);
            LOG_INFO("%s %d", "ser_fd --> ", ser_fd);
            LOG_INFO("%s %d", "pipe_fd[0] --> ", pipe_fd[0]);
            //如果是服务端套接字,接收客户端的连接
            if(sock_fd==ser_fd){
                LOG_INFO("%s", "如果是服务端套接字,接收客户端的连接");
                int client_fd;
            ...
            //如果是管道的一端有数据可读，那么处理信号
                ...
                LOG_INFO("%s", "如果是管道的一端有数据可读，那么处理信号");
                char signals[1024];
                int sig;
                int recv_ret_value;
